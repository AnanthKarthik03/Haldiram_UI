...
@Component({
  ...
  styleUrls: ['./customer-price-list.component.css'],
})
export class CustomerPriceListComponent implements OnInit {
  invoiceList = [];
  itemLists = [];
  ...
  itemOnChange(item) {
    ...
    this.service.customerPriceList(item).subscribe((data) => {
      if (data.success) {
